...
def handler(event, context):
    ...
    if vf:
        vf = "scale=" + vf
        exec_FFmpeg_cmd(['ffmpeg', '-y', '-i', input_path,
                         "-vf", vf, transcoded_filepath])
    else:
        # 音频
        if ac:
            if ar:
                exec_FFmpeg_cmd(
                    ['ffmpeg', '-y', '-i', input_path, "-ac", str(ac), "-ar", str(ar), transcoded_filepath])
            else:
                exec_FFmpeg_cmd(
                    ['ffmpeg', '-y', '-i', input_path, "-ac", str(ac), transcoded_filepath])

        else:
            if ar:
                exec_FFmpeg_cmd(
                    ['ffmpeg', '-y', '-i', input_path, "-ar", str(ar), transcoded_filepath])
            else:
                exec_FFmpeg_cmd(
                    ['ffmpeg', '-y', '-i', input_path, transcoded_filepath])
    return {}
